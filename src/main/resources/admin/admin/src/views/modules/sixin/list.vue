            ...
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '留言标题' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.sixinName" placeholder="留言标题" clearable></el-input>
                     </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('sixin','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/zonghexiaoqiguanlixitong/upload/sixinMuBan.xls"
                        >批量导入我的私信数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('sixin','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="sixinUploadSuccess"
                                :on-error="sixinUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('sixin','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入我的私信数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('sixin','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "sixin.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      ...
                                      label="留言内容">
                        <template slot-scope="scope">
                            <span v-if="scope.row.sixinText != null &&scope.row.sixinText.length>10">
                                {{scope.row.sixinText.slice(0,10)}}...
                            </span>
                            <span v-else>
                                {{scope.row.sixinText}}
                            </span>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('sixin','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('sixin','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('sixin','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'sixin',//当前表表名,
                        sumColum : 'sixin_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'sixin_name',//分组字符串字段
                        // types : 'sixin_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.sixinName!= '' && this.searchForm.sixinName!= undefined) {
                    params['sixinName'] = '%' + this.searchForm.sixinName + '%'
                }
                        
                params['sixinDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            sixinUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "sixin/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
