        ...
        <div v-if="showFlag">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <el-upload
                                ...
                                :action="getActionUrl"
                                :on-success="fangwuUploadSuccess"
                                :on-error="fangwuUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('fangwu','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "fangwu.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="fangwuLouhaoTypes"
                                      header-align="center"
                                      label="房屋楼号">
                        <template slot-scope="scope">
                            {{scope.row.fangwuLouhaoValue}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="fangwuDanyuanTypes"
                                      header-align="center"
                                      label="单元号">
                        <template slot-scope="scope">
                            {{scope.row.fangwuDanyuanValue}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('fangwu','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('fangwu','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('fangwu','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            form:{
                id : null,
                yonghuId : null,
                fangwuLouhaoTypes : null,
                fangwuDanyuanTypes : null,
                fangwuFanghao : null,
                fangwuDelete : null,
                insertTime : null,
                createTime : null,
            ...
            json_fields: {
                //级联表字段
                     ...
                     '余额': 'newMoney',
                //本表字段
                     '房屋楼号': "fangwuLouhaoValue",
                     '单元号': "fangwuDanyuanValue",
                     '房间号': "fangwuFanghao",
                     '录入时间': "insertTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'fangwu',//当前表表名,
                        sumColum : 'fangwu_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'fangwu_name',//分组字符串字段
                        // types : 'fangwu_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                params['fangwuDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            fangwuUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "fangwu/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
