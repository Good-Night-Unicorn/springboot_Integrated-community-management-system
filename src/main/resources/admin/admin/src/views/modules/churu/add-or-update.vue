    ...
    <div class="addEdit-block">
        <el-form
            ...
            <input id="yonghuId" name="yonghuId" type="hidden">
               <el-col :span="12">
                   <el-form-item class="input" v-if="type!='info'"  label="出入名称" prop="churuName">
                       <el-input v-model="ruleForm.churuName"
                                 placeholder="出入名称" clearable  :readonly="ro.churuName"></el-input>
                   </el-form-item>
                   <div v-else-if="type=='info'">
                       <el-form-item class="input" label="出入名称" prop="churuName">
                           <el-input v-model="ruleForm.churuName"
                                     placeholder="出入名称" readonly></el-input>
                       </el-form-item>
                   </div>
               </el-col>
                <el-col :span="12">
                    <el-form-item class="select" v-if="type!='info'"  label="出入类型" prop="churuTypes">
                        <el-select v-model="ruleForm.churuTypes" :disabled="ro.churuTypes" placeholder="请选择出入类型">
                            <el-option
                                v-for="(item,index) in churuTypesOptions"
                    ...
                    <div v-else-if="type=='info'">
                        <el-form-item class="input" label="出入类型" prop="churuValue">
                        <el-input v-model="ruleForm.churuValue"
                            placeholder="出入类型" readonly></el-input>
                        </el-form-item>
                    </div>
                </el-col>
                <el-col :span="12">
                    <el-form-item v-if="type!='info'"  class="input" label="出入时间" prop="churuTime">
                        <el-date-picker
                                value-format="yyyy-MM-dd HH:mm:ss"
                                v-model="ruleForm.churuTime"
                                type="datetime"
                                placeholder="出入时间"
                                :disabled="ro.churuTime">
                        </el-date-picker>
                    </el-form-item>
                    <div v-else-if="type=='info'">
                        <el-form-item v-if="ruleForm.churuTime" label="出入时间" prop="churuTime">
                            <span v-html="ruleForm.churuTime"></span>
                        </el-form-item>
                    </div>
                </el-col>

                <el-col :span="24">
                    <el-form-item v-if="type!='info'"  label="出入介绍" prop="churuContent">
                        <editor style="min-width: 200px; max-width: 600px;"
                                v-model="ruleForm.churuContent"
                                class="editor"
                                action="file/upload">
                        </editor>
                    </el-form-item>
                    <div v-else-if="type=='info'">
                        <el-form-item v-if="ruleForm.churuContent" label="出入介绍" prop="churuContent">
                            <span v-html="ruleForm.churuContent"></span>
                        </el-form-item>
                    </div>
                </el-col>
            </el-row>
...
<script>
    ...
    export default {
        data() {
            return {
                ...
                rules: {
                   ...
                   churuName: [
                              { required: true, message: '出入名称不能为空', trigger: 'blur' },
                          ],
                   churuTypes: [
                   ...
                   churuTime: [
                              { required: true, message: '出入时间不能为空', trigger: 'blur' },
                          ],
                   churuContent: [
                              { required: true, message: '出入介绍不能为空', trigger: 'blur' },
                          ],
                   churuDelete: [
        ...
        created() {
            ...
            //获取下拉框信息
                this.$http({
                    url:`dictionary/page?page=1&limit=100&sort=&order=&dicCode=churu_types`,
                    method: "get"
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.churuTypesOptions = data.data.list;
                    }
                });
        ...
        methods: {
            ...
            info(id) {
                ...
                _this.$http({
                    ...
                    if (data && data.code === 0) {
                        ...
                        _this.ruleForm.churuContent = _this.ruleForm.churuContent.replaceAll("src=\"upload/","src=\""+this.$base.url+"upload/");
                    } else {
            ...
            onSubmit() {
                this.$refs["ruleForm"].validate(valid => {
                    if (valid) {
                        this.ruleForm.churuContent = this.ruleForm.churuContent.replaceAll(this.$base.url,"");
                        this.$http({
                            url:`churu/${!this.ruleForm.id ? "save" : "update"}`,
                            method: "post",
                            data: this.ruleForm
                        }).then(({ data }) => {
                            if (data && data.code === 0) {
                                this.$message({
                                    ...
                                    onClose: () => {
                                        ...
                                        this.parent.churuCrossAddOrUpdateFlag = false;
                                        this.parent.search();
