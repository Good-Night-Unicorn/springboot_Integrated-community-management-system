        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '物业人员姓名' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.wuyeName" placeholder="物业人员姓名" clearable></el-input>
                     </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <el-upload
                                ...
                                :action="getActionUrl"
                                :on-success="wuyeUploadSuccess"
                                :on-error="wuyeUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('wuye','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入物业人员数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('wuye','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "wuye.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   ...
                                   label="物业人员身份证号">
                        <template slot-scope="scope">
                            {{scope.row.wuyeIdNumber}}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="wuyePhoto"
                                     header-align="center"
                                     width="200"
                                     label="物业人员头像">
                        <template slot-scope="scope">
                            <div v-if="scope.row.wuyePhoto">
                                <img :src="$base.url+scope.row.wuyePhoto" width="100" height="100">
                            </div>
                            <div v-else>无图片</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('wuye','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('wuye','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('wuye','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>

                            <el-button v-if="isAuth('wuye','修改')" type="success" icon="el-icon-tickets" size="mini" @click="resetPassword(scope.row.id)">重置密码</el-button>
                        </template>
                    </el-table-column>
                </el-table>
                <el-pagination
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'wuye',//当前表表名,
                        sumColum : 'wuye_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'wuye_name',//分组字符串字段
                        // types : 'wuye_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.wuyeName!= '' && this.searchForm.wuyeName!= undefined) {
                    params['wuyeName'] = '%' + this.searchForm.wuyeName + '%'
                }
                                                        
                params['wuyeDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            wuyeUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "wuye/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
