                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <el-upload
                                ...
                                :action="getActionUrl"
                                :on-success="cheweiFenpeiUploadSuccess"
                                :on-error="cheweiFenpeiUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('cheweiFenpei','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入车位分配数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('cheweiFenpei','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "cheweiFenpei.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
                ...
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('cheweiFenpei','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('cheweiFenpei','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('cheweiFenpei','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'chewei_fenpei',//当前表表名,
                        sumColum : 'chewei_fenpei_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'chewei_fenpei_name',//分组字符串字段
                        // types : 'chewei_fenpei_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "chewei_fenpei_types",
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                params['cheweiFenpeiDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "cheweiFenpei/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            cheweiFenpeiUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "cheweiFenpei/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
