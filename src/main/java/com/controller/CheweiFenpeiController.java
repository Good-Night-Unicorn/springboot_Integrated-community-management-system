...
public class CheweiFenpeiController {
    private static final Logger logger = LoggerFactory.getLogger(CheweiFenpeiController.class);

    private static final String TABLE_NAME = "cheweiFenpei";
    ...
    */
    ...
    public R page(@RequestParam Map<String, Object> params, HttpServletRequest request){
        ...
        PageUtils page = cheweiFenpeiService.queryPage(params);

        //字典表数据转换
        List<CheweiFenpeiView> list =(List<CheweiFenpeiView>)page.getList();
        for(CheweiFenpeiView c:list){
            //修改对应字典表字段
    ...
    */
    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(cheweiFenpei !=null){
            //entity转view
            CheweiFenpeiView view = new CheweiFenpeiView();
            BeanUtils.copyProperties( cheweiFenpei , view );//把实体数据重构到view中
            //级联表 车位
            //级联表
            CheweiEntity chewei = cheweiService.selectById(cheweiFenpei.getCheweiId());
            if(chewei != null){
            ...
            //级联表 用户
            //级联表
            YonghuEntity yonghu = yonghuService.selectById(cheweiFenpei.getYonghuId());
            if(yonghu != null){
    ...
    */
    ...
    public R update(@RequestBody CheweiFenpeiEntity cheweiFenpei, HttpServletRequest request) throws NoSuchFieldException, ClassNotFoundException, IllegalAccessException, InstantiationException {
        logger.debug("update方法:,,Controller:{},,cheweiFenpei:{}",this.getClass().getName(),cheweiFenpei.toString());
        CheweiFenpeiEntity oldCheweiFenpeiEntity = cheweiFenpeiService.selectById(cheweiFenpei.getId());//查询原先数据
...
//            cheweiFenpei.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
            ...
            cheweiFenpeiService.updateById(cheweiFenpei);//根据id更新
            return R.ok();
    }
    ...
    */
    ...
    public R save( String fileName, HttpServletRequest request){
        ...
        try {
            List<CheweiFenpeiEntity> cheweiFenpeiList = new ArrayList<>();//上传的东西
                ...
                if(!".xls".equals(suffix)){
                    ...
                    if(!file.exists()){
                        ...
                        for(List<String> data:dataList){
                            //循环
                            CheweiFenpeiEntity cheweiFenpeiEntity = new CheweiFenpeiEntity();
//                            cheweiFenpeiEntity.setCheweiId(Integer.valueOf(data.get(0)));   //车位 要改的
//                            cheweiFenpeiEntity.setYonghuId(Integer.valueOf(data.get(0)));   //住户 要改的
//                            cheweiFenpeiEntity.setFenpeiTime(sdf.parse(data.get(0)));          //分配时间 要改的
//                            cheweiFenpeiEntity.setCreateTime(date);//时间
                            cheweiFenpeiList.add(cheweiFenpeiEntity);
                        ...
                        //查询是否重复
                        cheweiFenpeiService.insertBatch(cheweiFenpeiList);
                        return R.ok();
                    }
                }
            }
